...
async function refreshAccessToken(refreshToken) {
    ...
    if (!newAt) {
        // Logujeme jen názvy cookies – hodnoty jsou tokeny
        const raw = response.headers.raw?.()?.['set-cookie'] || [response.headers.get('set-cookie') || '']
        const names = raw.map(h => h.split('=')[0].trim()).filter(Boolean)
        console.error('[herohero] Set-Cookie obsahoval pouze:', names.join(', ') || '(nic)')
